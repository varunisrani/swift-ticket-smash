...
### Step 4: Task Execution Loop with Specialized Agent Routing

**IMPORTANT**: This is a loop. Execute ALL assigned tasks before proceeding to Phase 3.

Execute all assigned parent tasks and their subtasks using @.agent-os/instructions/core/execute-task.md instructions with specialized agent routing, continuing until all tasks are complete.

<specialized_agents_available>
  <agent-generator>
    PURPOSE: Creates new specialized agents for specific tasks or domains
    USE FOR: Agent creation, workflow automation design
  </agent-generator>

  <ai-engineer>
    PURPOSE: AI/ML integrations, LLM APIs, multi-agent systems
    USE FOR: OpenAI/Gemini integration, CrewAI setup, agent orchestration
  </ai-engineer>

  <frontend-specialist>
    PURPOSE: React/Next.js UI, responsive design, dashboards
    USE FOR: Component development, admin pages, editable interfaces
  </frontend-specialist>

  <error-resolver>
    PURPOSE: Debug and fix development errors quickly
    USE FOR: Build errors, API issues, JSON parsing, hydration problems
  </error-resolver>

  <build-troubleshooter>
    PURPOSE: Fix build failures through iterative troubleshooting
    USE FOR: npm issues, webpack config, deployment problems, CI/CD
  </build-troubleshooter>
</specialized_agents_available>
...
<execution_flow>
  ...
  FOR each parent_task assigned in Step 1:
    ANALYZE: Task requirements for specialized agent needs

    IF task involves UI/React components:
      CONSIDER: frontend-specialist agent
    ELSE IF task involves AI/ML/LLM integration:
      CONSIDER: ai-engineer agent
    ELSE IF task involves creating new agents:
      CONSIDER: agent-generator agent
    ELSE IF encountering errors during execution:
      USE: error-resolver or build-troubleshooter as needed

    EXECUTE instructions from execute-task.md with:
      - parent_task_number
      - all associated subtasks
      - specialized agent routing when beneficial
...
<agent_selection_criteria>
  <proactive_routing>
    - Analyze task description for keywords indicating specialization
    - Route to specialized agent BEFORE attempting standard execution
    - Use multiple agents in parallel when tasks are independent
  </proactive_routing>

  <reactive_routing>
    - If errors occur: Deploy error-resolver immediately
    - If build fails: Deploy build-troubleshooter for iterative fixes
    - If UI breaks: Deploy frontend-specialist for repairs
  </reactive_routing>
</agent_selection_criteria>
...
<instructions>
  ...
  ANALYZE: Each task for specialized agent requirements
  ROUTE: Tasks to appropriate specialized agents proactively
  REUSE: Same instructions for each parent task iteration
  LOOP: Through all assigned parent tasks
  UPDATE: Task status after each completion
  VERIFY: All tasks complete before proceeding
  HANDLE: Blocking issues with appropriate agents
